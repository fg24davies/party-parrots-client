...
const ParrotPage = ({ route, navigation }) => {
  ...
  useEffect(() => {
    ...
    const fetchParrot = async () => {
      const res = await fetch(
        `https://parrot-party-api.herokuapp.com/api/parrots/${parrotId}`,
        {
          method: "GET",
        }
      )
  ...
  const isUserInApplicants = () => {
    const userApplication = applications.filter(
      (application) => application.user._id === userId
    );
    return userApplication.length > 0;
  };

  const isParrotApproved = () => {
    const approvedApplication = applications.filter(
  ...
  const isApprovedUser = () => {
    ...
    return (
      ...
      approvedApplication[0].user._id === userId
    );
  };
    ...
    <ScrollView>
      <View style={styles.pageBody}>
        ...
        <View style={styles.parrotInfo}>
          ...
          <ParrotInfoItem
            label="Special Needs: "
            info={parrot.specialNeeds}
            compact="false"
          />
        </View>
        <View style={styles.applicationsContainer}>
          {userType !== "admin" &&
            isUserInApplicants() === false &&
            !isParrotApproved() && (
